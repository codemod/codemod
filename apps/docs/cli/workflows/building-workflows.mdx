...
<ParamField path="if" type="string">
    ...
    Example: <code>params.autoAIReview</code>.
  </ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Shell command step">
    ...
    <ParamField path="env" type="object">
      Step-level environment variables applied to the process environment for this <code>run</code> step.
    </ParamField>
  </Accordion>

  <Accordion title="Template step">
  ...
  <Accordion title="Codemod Registry step">
    ...
    <ParamField path="env" type="object">
      Step-level environment variables. For <code>codemod</code> steps, these are forwarded to the invoked workflow as parameters with an <code>env_</code> prefix (e.g., <code>FOO</code> → <code>env_FOO</code>). They are not applied to the OS environment of the nested workflow.
    </ParamField>
  </Accordion>
      ...
      steps:
        - name: Review diffs with AI
          ai:
            ...
            model: "gpt-4o"                           # model
            api_key: "YOUR_API_KEY_HERE"               # paste key inline
      ```
