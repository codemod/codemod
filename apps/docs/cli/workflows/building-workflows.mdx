...
Steps are the atomic actions inside a node; they run sequentially and each step performs one action (`run`, `use`, `ast-grep`, `js-ast-grep`, `codemod`, or `ai`).
...
<Tabs>
  <ParamField path="if" type="string">
    Conditional expression to gate the step.
   ...
   <ParamField path="ai.max_steps" type="number" default="30">
    ...
    <ParamField path="ai.api_key" type="string">
      ...
      <Note>
        ...
        Step-level <code>env</code> and <code>ai.env</code> are currently ignored by the engine.
      </Note>
    </ParamField>
