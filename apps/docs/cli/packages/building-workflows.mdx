...
<Info>
The Codemod app provides centralized state management for large-scale refactoring across many repos. See [i18n automation](https://codemod.com/i18n) for examples of coordinated multi-repo migrations.
</Info>
...
**Access parameters in jssg codemods via `options.params`:**
...
export default function transform(root: SgRoot<TSX>, options) {
  ...
  // Access matrix values (when using matrix strategy)
  if (options.matrixValues?.shard && options.matrixValues._meta_files) {
    // This task is part of a matrix - use shard info
    const fits = fitsInShard(filename, options.matrixValues);
  }
}
```

{/* <Tip>
For comprehensive examples of parameters, matrix strategy, and enterprise integration patterns, see the [jssg Advanced guide](/jssg/advanced).
</Tip> */}

<Note>
**Parameters vs Matrix Values:** `options.params` contains user-configured workflow settings, while `options.matrixValues` contains values from matrix strategy (e.g., `team`, `shard`, `shardId` from the `shards` state array). See [Matrix Strategy](#matrix-strategy) for details.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="JS ast-grep (jssg) step">
    ...
    <Tip>
      New to jssg? Start with the <a href="/jssg/quickstart">jssg quickstart</a> to scaffold a package and write your first transform.
    </Tip>
    ```yaml
    steps:
