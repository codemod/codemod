...
<Steps>
  <Step title="Open Migration Campaigns">
    ...
    <Note>
      The Campaigns section is visible only on [pro tier or above](https://codemod.com/pricing) or upon request.
    </Note>
  </Step>
...
While a campaign may be as simple as a single automated codemod, most migrations involve a sequence of steps. You can design those steps by building [Codemod Packages](/cli/packages). Pro and enterprise customers can request custom workflows.

<Card title="Learn more about codemod packages →" icon="terminal" href="/cli/packages" />
