...
const transform: Transform<TSX> = (root) => {
  ...
  if (def) {
    ...
    console.log("Definition kind:", def.kind); // 'local', 'import', or 'external'
    
    // Handle different definition kinds
    if (def.kind === 'import') {
      console.log("Symbol comes from an unresolved import");
    } else if (def.kind === 'external') {
      console.log("Symbol defined in another file");
    } else {
      console.log("Symbol defined locally");
    }
  } else {
...
When you need to trace symbols from external modules that can't be resolved:
...
const transform: Transform<TSX> = (root) => {
  ...
  // Find a symbol that might come from an external module
  const symbol = rootNode.find({ rule: { pattern: "externalSymbol" } });
  if (!symbol) return null;
  
  // By default, definition() tries to resolve imports
  const def = symbol.definition();
  
  if (def && def.kind === 'import') {
    // The import couldn't be resolved (e.g., node_modules not installed)
    // but we still have the import statement
    console.log("Symbol imported from:", def.node.text());
  }
  
  // Or explicitly stop at imports without trying to resolve
  const importDef = symbol.definition({ resolveExternal: false });
  if (importDef) {
    // Always returns the import statement for imported symbols
    console.log("Import statement:", importDef.node.text());
  }
