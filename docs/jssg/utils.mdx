...
<Info>
These utilities work with JavaScript, TypeScript, JSX, and TSX ASTs. They handle both ESM (`import`) and CJS (`require()`) formats automatically, including dynamic `import()` patterns that bind values through `.then(...)` callbacks.
</Info>
...
| Dynamic `.then()` namespace callback | `import("module").then((mod) => mod.fn())` |
| Dynamic `.then()` destructured named callback | `import("module").then(({ fn }) => fn())` |
| Dynamic `.then()` destructured alias callback | `import("module").then(({ fn: myFn }) => myFn())` |
...
- **Merges** named specifiers into destructured dynamic `import().then(...)` callbacks from the same source
...
- Handles dynamic `import()` forms, including `.then(...)` callback bindings.
...
### Work With Dynamic `import().then(...)`

These helpers also treat `.then(...)` callback bindings as import-like declarations when the source module matches:

```typescript
const modImport = getImport(rootNode, { type: "default", from: "mod" });
// Matches: import("mod").then((mod) => { mod.fn(); })

const namedImport = getImport(rootNode, { type: "named", name: "foo", from: "mod" });
// Matches: import("mod").then(({ foo }) => { foo(); })
...
// Turns: import("mod").then(({ foo }) => { foo(); })
// Into:  import("mod").then(({ foo, bar }) => { foo(); })
...
// Turns: import("mod").then(({ foo, bar }) => { bar(); })
// Into:  import("mod").then(({ bar }) => { bar(); })
```
