...
| Dynamic import (destructured) | `const { bar } = await import("module")` |
...
<ParamField body="options" type="RemoveImportOptions" required>
  One of:
  - `{ type: "default", from: string, removeSideEffectForms?: boolean }` — Remove the default import from a module. If no bound default import exists, `removeSideEffectForms: true` also removes bare `import "module"` and `require("module")` statements for the same source.
