...
Behavior notes:
- Returns every matching import for the requested module, across both ESM and CJS forms.
- If no typed match exists, namespace imports such as `import * as ns from "pkg"` are returned as a fallback.
- When a typed match does exist, namespace fallbacks are omitted.
...
- **Default/namespace**: Removes the entire import statement unless it is part of a mixed import, in which case the remaining sibling import is preserved.
- **Named (multiple specifiers)**: Removes only the specified specifiers, keeping the rest.
- **Named (last specifier)**: Removes the entire import statement unless the statement also has a default import, in which case only the named clause is removed.
- Handles both ESM and CJS formats.
- Returns `null` instead of deleting unrelated bindings from multi-declarator `require()` statements.
- Can also remove side-effect-only forms such as `import "pkg"` and `require("pkg")` when `removeSideEffectForms: true` is provided.
...
<ParamField body="options" type="RemoveImportOptions" required>
  One of:
  - `{ type: "default", from: string, removeSideEffectForms?: boolean }` — Remove the default import from a module.
...
To also remove side-effect-only imports, pass `removeSideEffectForms: true`:

```typescript
const removeSideEffectEdit = removeImport(rootNode, {
  type: "default",
  from: "core-js/stable",
