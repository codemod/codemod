...
Use these when Java codemods need to prove framework ownership, handle wildcard/conflicting imports, avoid shadowed receivers, replace type identifiers without touching fully qualified names, or convert anonymous callback classes safely.

```typescript
const imports = collectJavaImports(rootNode);

const isSpringResponseEntity =
