...
- what are the semantic child nodes here, without punctuation or comment trivia?
- is this node effectively inside a `new (...)` constructor position?
- is this usage hidden behind extra parentheses?
- is this value being used in reflective/introspection access such as `["name"]` or `"prototype" in value`?

These helpers are intended for conservative usage-context gating.

## `getNamedChildren`

Return the node's named children while skipping non-named tokens and comment nodes.

Use this when a codemod needs semantic child nodes instead of punctuation, delimiters, or trivia.

## `unwrapParenthesizedExpression`

Return the innermost expression after stripping `parenthesized_expression` wrappers.

## `isUsedAsConstructor`

Return `true` when a node is effectively used as the `constructor` of a `new_expression`, even through transparent wrappers.
...
Return `true` when a node is used in one of these reflective forms for one of the requested keys:
