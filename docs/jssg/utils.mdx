...
Use these helpers when a codemod needs to answer questions like:

- does this usage really refer to the imported runtime symbol?
- is this binding type-only?
- is a local declaration shadowing the imported name?
- is a JSX tag like `<Grid />` actually the imported component or a local shadow?

These helpers are intended for conservative transform gating.

## `getTopLevelImportBinding`

Return the first matching top-level import binding or `null`.
...
<ParamField body="options" type='{"type":"default","from":string} | {"type":"named","name":string,"from":string}' required>
  What top-level import binding to look for.
</ParamField>

### Returns

`ImportBinding<T> | null` — an object containing:

| Field | Type | Description |
|-------|------|-------------|
| `alias` | `string` | The local name used at call sites |
| `isNamespace` | `boolean` | `true` for namespace imports |
| `moduleType` | `"esm" \| "cjs"` | Whether the binding came from ESM or CJS |
| `node` | `SgNode<T, "identifier">` | The identifier AST node for the binding |
| `isTypeOnly` | `boolean` | `true` for `import type` or inline `type` specifiers |

### Example

```typescript
const binding = getTopLevelImportBinding(rootNode, {
  type: "named",
  name: "Grid",
  from: "@mui/material",
});
...
if (binding && !binding.isTypeOnly) {
  console.log(`Runtime Grid imported as: ${binding.alias}`);
}
```
...
Return every matching top-level import binding for a module/symbol pair.

Use this when a file may contain multiple aliased or mixed type/runtime imports that all need inspection.

### Example

```typescript
const bindings = getAllTopLevelImportBindings(rootNode, {
  type: "named",
  name: "Grid",
  from: "@mui/material",
});
...
Return `true` when the given binding node belongs to:
...
Return the local binding node that shadows an identifier at the given usage site, or `null` when no local shadow exists.
...
This also works for JSX tag identifiers:
...
if (
  ...
  // Safe to treat <Grid /> as the imported runtime component.
}
```
...
This is a small conservative helper for direct identifier checks.

---

## `isRuntimeImportBinding`

Return `true` when the given usage node resolves to a non-type-only top-level import matching the query.

Use this as the main guard before rewriting runtime symbol usages.
