...
- `codemod:llm` — engine-owned text generation for codemods that declare the `fetch` capability
...
<ParamField path="definition(options?)" type="DefinitionResult | null">Get the definition for the symbol at this node's position. Returns an object with `node` (the definition SgNode), `root` (the SgRoot for the file containing the definition), and `kind` (`local`, `import`, or `external`), or `null` if not found.</ParamField>
...
<ParamField path="options.resolveExternal" type="boolean" default="true">When `false`, stop at the import statement instead of attempting to resolve the imported module to another workspace file.</ParamField>
...
```ts
import generate, { type LlmRequest, type LlmResponse } from "codemod:llm";
```

Use `codemod:llm` when your codemod needs engine-provided text generation. The engine owns provider credentials, model selection, and usage accounting.
...
<ParamField path="outputSchema" type="Record&lt;string, unknown&gt; | boolean">
...
<ParamField path="LlmResponse" type="object">
...
<ParamField path="generate(request)" type="(request: LlmRequest) => Promise&lt;LlmResponse&gt;">
  Generates text through the engine-owned LLM client.
</ParamField>

### Notes

- Your codemod must declare the `fetch` capability before `codemod:llm` can be used.
- Provider credentials and model selection come from the engine, not from your codemod.
