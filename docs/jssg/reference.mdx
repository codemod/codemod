...
<Info>
Language kinds and editor IntelliSense are available via [codemod:ast-grep/langs/*](https://github.com/codemod/codemod/tree/main/packages/jssg-types/src/langs). For example, use `import type TSX from "codemod:ast-grep/langs/tsx"` for TSX or `import type XML from "codemod:ast-grep/langs/xml"` for XML.
</Info>
