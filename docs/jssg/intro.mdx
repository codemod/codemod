...
- **Structured runtime logs and progress** — `console.log`, `console.warn`, and the [`codemod:runtime`](/jssg/reference#runtime-hooks) hooks surface buffered progress and warning events to task logs and diagnostics, including when emitted from handler code during execution.
