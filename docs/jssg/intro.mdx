...
- **Not only JavaScript** — jscodeshift targets JS/TS. JSSG is built on ast-grep and Tree-sitter, so the same tool and workflow can cover Python, Rust, Go, Java, XML, and other languages Tree-sitter supports—one system for humans and agents to learn.
