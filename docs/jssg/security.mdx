...
<CardGroup cols={2}>
  <Card title="Inside target_dir" icon="folder-open">
    **Allowed by default**
    ...
    `readFileSync`, `writeFileSync`, `readdirSync`, `mkdirSync`, `statSync`, `existsSync`, `accessSync`, `unlinkSync`, and their `fs/promises` equivalents all work. When you pass `{ withFileTypes: true }` to `readdirSync` or `fs.promises.readdir`, the sandboxed `fs` returns Node-style `Dirent` objects instead of plain strings.
  </Card>
