...
<Info>
  When a workflow step runs a nested registry codemod, Codemod merges capabilities from the current run and the nested package's `codemod.yaml`. A child package can request additional capabilities it needs, and a parent package or CLI flags can still grant broader access for the full run.
</Info>
