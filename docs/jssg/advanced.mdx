...
- `options.dryRun: boolean` — whether the current execution is running in preview mode. Use it when you need different logging or reporting behavior for dry runs.
...
<Info>
In `dryRun` mode, sandboxed filesystem operations still run against the in-memory preview filesystem. That means `fs.writeFileSync`, `fs.unlinkSync`, and parser-backed writes to files under `options.targetDir` can affect the preview result without mutating files on disk.
</Info>
...
<a href="/cli#codemod-workflow">Passing parameters (--param)</a>.
</Tip>
