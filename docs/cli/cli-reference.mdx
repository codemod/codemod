...
<Info>
  Package metadata (name, version, access level, etc.) is read from `codemod.yaml`. See [Building Workflows](/cli/packages/building-workflows#package-metadata-codemod-yaml) for configuration options.
</Info>
