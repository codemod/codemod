...
<ParamField path="shard.js-ast-grep" type="object">
  JSSG codemod configuration for pre-filtering. When set, the engine dry-runs the codemod and only shards files where the transform produces changes. See [JSSG step parameters](/community/workflows/reference#jssg-step) for the full field reference.
</ParamField>
