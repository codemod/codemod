...
<Card title="Quickstart" icon="rocket" href="/community/oss-quickstart">
...
Manage and execute [Codemod packages](/community/package-structure).
...
**Learn more**: [Workflow Reference](/community/workflows/reference)
...
<ResponseField name="--llm-usage-output <PATH>" type="string">
  Write provider-reported nested LLM usage for the current package run to a JSON file.

  This output is only populated for LLM requests made by nested codemods during `npx codemod <package-name>` or other package-run flows. The file contains aggregated request counts by provider, model, outcome, and token-usage bucket, and it does not include prompts, API keys, or other request payload data.

  ```bash
  # Capture nested LLM usage while running a registry package
  npx codemod @org/my-package --llm-usage-output ./artifacts/llm-usage.json

  # Capture nested LLM usage for a local package run
  npx codemod workflow run -w ./my-codemod --llm-usage-output ./artifacts/llm-usage.json
  ```

  The CLI creates parent directories as needed and overwrites the file with the current run's data.
</ResponseField>
...
Run [jssg (JS ast-grep)](/community/jssg/intro) transforms directly without a workflow.
...
<Tip>
  For production, embed your jssg transform in a workflow using a `js-ast-grep` step. See [Workflow Reference: JSSG Step](/community/workflows/reference#jssg-step).
</Tip>

**Learn more**: [jssg documentation](/community/jssg/intro)
...
Both workflows use [trusted publishers](/community/publishing#trusted-publishers) with OIDC for secure, passwordless publishing.

[See the publishing guide](/community/publishing#complete-github-actions-workflow) for detailed workflow examples.
...
<Info>
  Package metadata (name, version, access level, etc.) is read from `codemod.yaml`. See [Package Structure](/community/package-structure#package-metadata-codemod-yaml) for configuration options.
</Info>

<Card title="Publishing Guide" icon="upload" href="/community/publishing">
