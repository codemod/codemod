---
title: "JSSG"
description: "Write JavaScript/TypeScript AST transformations with Codemod's JSSG engine."
---

On this page, you will learn what JSSG is and how it works, see a sample codemod, and understand how it differs from typical JavaScript-only toolkits. To scaffold a package, run tests, and publish, use the [Quickstart](/community/oss-quickstart).
...
<Info>
For engine details and built-ins, see <a href="/community/jssg/reference#runtime-and-built-ins">Runtime and built-ins</a> in the API reference.
</Info>
...
You can author rules as plain JavaScript objects. See the [ast‑grep rule configuration docs](https://ast-grep.github.io/guide/rule-config.html) and the [JSSG API reference](/community/jssg/reference#rule-configuration).
...
- **A small, direct API** — Large, loosely typed, chain-heavy AST APIs are easy for LLMs to misuse. JSSG stays close to the tree: `find`, `findAll`, `replace`, `text()`, and related helpers—less surface area, fewer invented methods, less time debugging transforms that never could have worked. See the [JSSG reference](/community/jssg/reference).
- **MCP: agents can inspect real ASTs** — Through [Codemod MCP](/community/model-context-protocol), an agent can dump the AST of a snippet, browse Tree-sitter node types, and run codemod tests inside the conversation. It does not have to guess what the tree looks like; that feedback loop is what makes agent-driven codemods reliable instead of hopeful.

### Runtime & Execution Model

*How codemods run: safety, observability, and power during execution.*

- **Deny-by-default security** — No file system, child processes, or network access unless you grant [capabilities](/community/jssg/security)—unlike typical Node transform runners where full `fs` and `require` are the default.
- **Metrics for mining, not only edits** — Use [`codemod:metrics`](/community/jssg/metrics) to count and aggregate findings (usage, readiness, rollouts) with the same harness you use for rewrites.
- **Import and edit utilities** — [`@jssg/utils`](/community/jssg/utils) and similar helpers cover tedious import and AST chores without custom Babel plugins.
- **Multi-file and file-path changes** — Use [`jssgTransform`](/community/jssg/reference#multi-file-transforms-with-jssgtransform) to drive coordinated edits across files, and [`root.rename()`](/community/jssg/reference#file-renaming) for moves and extension changes—not only in-place source edits.

### Platform & Distribution

*Everything beyond writing a transform: shipping, orchestration, and operating migrations at scale—not “just a toolkit.”*

- **Registry** — Publish a package to the [Codemod Registry](/community/publishing) so anyone can run it with `npx codemod @scope/name`—no cloning or local setup for consumers.
- **Workflow engine** — Real migrations are rarely a single pass. Compose JSSG steps in YAML with deterministic stages, optional AI-assisted steps where patterns are not enough, and shared state between steps; the engine handles orchestration, [sharding across PRs](/community/workflows/sharding), and progress. See the [Workflow reference](/community/workflows/reference).
- **Beyond the CLI** — For large organizations, the [Codemod Platform](/enterprise/quickstart) builds on the same analysis: [Insights](/enterprise/insights) for adoption and impact, [Campaigns](/enterprise/campaigns) to split work across teams and repos, and enterprise workflows so you are not landing one enormous PR across every owner at once.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/community/jssg/semantic-analysis"
