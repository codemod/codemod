---
title: "Workflows"
description: "Orchestrate automated code transformations at scale with Codemod Workflows."
---
...
In Campaign/cloud runs, a node can also drive git automation: you can create a task branch before the node runs, create commits after successful steps, and open a pull request when the node finishes with commits. See [Workflow Reference](/community/workflows/reference#nodes).
...
| Step Type | Purpose |
|-----------|---------|
| `js-ast-grep` | JavaScript/TypeScript codemods using [JSSG](/community/jssg) |
...
| `shard` | [Split migrations into multiple PRs](/community/workflows/sharding) by directory, codeowner, or custom logic |
...
<CardGroup cols={2}>
  <Card title="Workflow Reference" icon="book" href="/community/workflows/reference">
    Complete workflow.yaml specification with all options.
  </Card>
  <Card title="AI step" icon="sparkles" href="/community/workflows/ai-harness-rig">
    How the `ai` step runs under Codemod's Rig-based harness.
  </Card>
  <Card title="Sharding" icon="layer-group" href="/community/workflows/sharding">
    Split migrations into multiple PRs by directory, codeowner, or custom logic.
  </Card>
  <Card title="JSSG" icon="code" href="/community/jssg">
    Write JavaScript/TypeScript transformations with ast-grep.
  </Card>
</CardGroup>
