...
This guide covers all the ways to publish codemods to the [Codemod Registry](/enterprise/registry), from local development to automated CI/CD pipelines.
...
<sub>* Verifying an organization scope is only part of setup. Some packages also require adding a Trusted Publisher entry in the UI (for example, unscoped packages or when the GitHub org doesn't match the package scope). See [Organization Scopes](#organization-scopes) and [Individual Packages (Manual Configuration)](#individual-packages-manual-configuration).</sub>
...
<CardGroup cols={2}>
  ...
  <Card title="Customize Dashboard Templates" icon="chart-simple">
    If your package includes JSSG steps, it ships a default Insights dashboard. [Update the dashboard template](/enterprise/insights#dashboard-templates) so consumers start from your recommended widgets.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Package Structure" icon="folder-tree" href="/community/package-structure">
    Create and configure Codemod packages.
  </Card>

  <Card title="CLI Reference" icon="terminal" href="/community/cli#codemod-publish">
