...
- **Scan Codebase** runs an ast-grep, JSSG, or Smart query against selected repositories.
...
The **Scan Codebase** action uses the same query model as [Insights](/enterprise/insights). It can run ast-grep rules, JSSG queries, or [Smart](/enterprise/insights/smart-queries) natural-language file checks.

The action's **Result** setting, not the query source, decides the shape of the output:

- **Scalar** sets `scan.totalCount` and leaves `scan.rows` empty.
- **Table** sets `scan.rows` to the grouped rows and `scan.totalCount` to their sum.

Smart measures file counts only, so a Smart `scan.totalCount` is always a file count, while ast-grep can also count matches. Use scan output in later templates to decide whether to create a campaign, send a webhook, or ask a coding agent to work only when the query finds matching code.
...
<CardGroup cols={3}>
  ...
  <Card title="Jira integration" icon="jira" href="/enterprise/integrations/jira">
