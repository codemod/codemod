...
A Campaign allows you to orchestrate large-scale code migrations. When you create a Campaign, Codemod executes the underlying [workflow](/community/workflows), creates tasks, and opens pull requests. You can monitor progress, review PRs, re-run failed tasks, and track status from a centralized dashboard.

## How Campaigns use workflows

Campaigns are powered by the Codemod [workflows](/community/workflows), which orchestrates multi-step code migrations at scale.
...
- **Tasks** are runtime instances of nodes. When a Campaign runs, the engine creates tasks for each node. For [matrix strategies](/community/workflows/reference#matrix-strategy), a single node can spawn multiple parallel tasks (e.g., one per team or shard).

In Codemod Campaigns, each task executes on its own branch. Nodes can define step-level commit checkpoints and pull request metadata, and a task that ends with commits will push its branch and open a pull request. See [Workflow reference](/community/workflows/reference#nodes) and [available step fields](/community/workflows/reference#common-step-fields).
...
<Steps>
  ...
  <Step title="Select target repositories">
    ...
    <Info>
      Codemod needs access to your repositories via the [GitHub App](/enterprise/integrations/github) or [GitLab integration](/enterprise/integrations/gitlab). If no repositories appear, connect your account first.
    </Info>
  </Step>
  ...
  <Step title="Configure parameters">
    ...
    - **JSSG transforms**: Access parameters via `options.params`. See [JSSG documentation](/community/jssg/advanced#parameters-and-configuration) for details.
  ...
  <Step title="Configure rollout behavior">
    Depending on the selected package and workflow, campaign creation can include additional rollout controls:

    - **Sharding** splits eligible codemods into smaller parallel tasks and pull requests. You can group files by `.github/CODEOWNERS` or directory structure, set the maximum files per shard, and optionally set a minimum shard size or target directory.
    - **AI review** adds a final cleanup step when the workflow does not already include an AI step.
...
<CardGroup cols={2}>
  ...
  <Card title="GitLab Integration" icon="gitlab" href="/enterprise/integrations/gitlab">
...
When sharding is enabled, Codemod splits eligible work into smaller parallel tasks. Shards can be grouped by code owner or directory, and each shard follows the same workflow execution model as any other task.

Use sharding when one large migration PR would be difficult to review or when teams need smaller slices of the same migration. The table view is usually the clearest way to inspect sharded work because each matrix task appears as its own row.
...
Generated pull requests show supported CI checks inline on the task card. Codemod groups checks into pending, passing, and failing states and links to provider check details when the provider sends a URL.

Campaigns also include auto-heal for generated PRs. When auto-heal is on, Codemod waits for checks to settle before deciding whether recovery is needed. It can then:

- Patch the generated PR directly when the failure looks repository-specific.
...
You can turn auto-heal on or off from the campaign detail page. When it is off and checks settle with failures, Codemod records that auto-heal is available and lets you start it manually from the pull request card.

Recovery status appears inline with the PR checks. When available, use **View agent** or **View logs** to inspect the recovery work.
...
<CardGroup cols={2}>
  <Card title="Set up Jira" icon="jira" href="/enterprise/integrations/jira">
...
<Card title="Parameter Reference" icon="sliders" href="/community/workflows/reference#parameters">
...
<CardGroup cols={2}>
  <Card title="Enterprise" icon="play" href="/enterprise/quickstart">
    Run your first codemod and create a PR in minutes.
  </Card>
  <Card title="Workflows" icon="diagram-project" href="/community/workflows">
