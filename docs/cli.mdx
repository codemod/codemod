...
Manage and execute [Codemod packages](/workflows/quickstart).
...
**Learn more**: [Building Workflows](/workflows/building-workflows)
...
<Tip>
  For production, embed your jssg transform in a workflow using a `js-ast-grep` step. See [Building Workflows: JS ast-grep step](/workflows/building-workflows#js-ast-grep-step).
</Tip>
...
When you run `codemod init`, you'll be prompted to create a GitHub Actions workflow for publishing. If you accept, it creates `.github/workflows/publish.yml` with:
...
Both workflows use [trusted publishers](/publishing#trusted-publishers) with OIDC for secure, passwordless publishing. See the [Publishing guide](/publishing#complete-github-actions-workflow) for detailed workflow examples.
...
<Info>
  Package metadata (name, version, access level, etc.) is read from `codemod.yaml`. See [Building Workflows](/workflows/building-workflows#package-metadata-codemod-yaml) for configuration options.
</Info>
