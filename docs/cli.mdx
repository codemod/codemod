...
<ResponseField name="-i, --id <ID>" type="string">
  Open the TUI focused on a specific workflow run.
</ResponseField>

<ResponseField name="-l, --limit <LIMIT>" type="number">
  Number of workflow runs to load in the TUI list. (default: 20)
</ResponseField>
...
Execute a JavaScript or TypeScript file directly in the Codemod sandbox.

```bash
npx codemod jssg exec <js_file>
```

<ResponseField name="js_file" type="string" required>
  Path to the JavaScript or TypeScript file to execute.
</ResponseField>

Use this command when you want to run a sandboxed script directly rather than apply a codemod over a target directory.
...
<ResponseField name="--usage-log <PATH>" type="string">
  Write MCP usage events to a file for local debugging.
</ResponseField>

<ResponseField name="--allow-feedback" type="boolean">
  Opt in this OS user to anonymous Codemod AI and MCP feedback collection before the server starts.
</ResponseField>

<Info>
  This command is typically launched by an MCP client configuration rather than run manually in an interactive shell.
</Info>
