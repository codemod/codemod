...
<ResponseField name="--format <FORMAT>" type="string">
  ...
  JSON and YAML output include two extra summary fields in addition to `total` and `packages`:
  - `metadata_coverage`: summarizes how completely the matched packages populate discoverability metadata such as descriptions, categories, frameworks, and languages.
  - `threshold_route_preview`: shows the CLI's suitability routing preview for the current result set.
</ResponseField>
...
<Accordion title="Examples">
  ...
  ```json
  {
    ...
    "metadata_coverage": {
      ...
      "framework_coverage": 0.5,
      "language_coverage": 1.0
    },
    "threshold_route_preview": "registry"
  }
  ```
</Accordion>
