...
<Info>
  `ai docs` is the user-facing command for docs-backed MCP resources. Use `ai tools` and `ai tool` to inspect tool metadata, and use `ai call` to invoke a tool directly from the CLI.
</Info>
