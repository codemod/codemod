...
Use this command to inspect which Codemod-managed skills are currently installed for the selected harness. This is also the command referenced by generated harness guidance after `codemod ai` installs or updates managed integrations.

<ResponseField name="--harness <claude|goose|opencode|cursor|codex|auto>" type="string">
  Target harness adapter. Defaults to `auto`.
</ResponseField>

<ResponseField name="--format <logs|table|json|yaml>" type="string">
  Output format. Defaults to `logs`.
</ResponseField>
