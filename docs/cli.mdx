...
Inspect MCP-equivalent resources exposed by the CLI.
...
- `ai resources` lists the available resource names, URIs, and descriptions.
- `ai resource <uri-or-name>` reads one resource by full URI or by its short name.

```bash
# List resources exposed through the CLI
npx codemod ai resources

# Read one resource by short name
npx codemod ai resource codemod-cli-instructions

# Read one resource by URI
npx codemod ai resource codemod://docs/jssg-instructions
```
