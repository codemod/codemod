...
Use this command when you want an MCP-compatible client or harness to talk to the local Codemod CLI process directly.
...
<ResponseField name="--allow-feedback" type="boolean">
  Opt in this OS user to anonymous Codemod AI and MCP feedback when starting the server.
</ResponseField>

<Info>
  `codemod ai` configures supported harnesses to run this command automatically through their MCP settings. You usually run `codemod mcp` directly only when testing or integrating a client by hand.
</Info>
