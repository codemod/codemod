...
<ResponseField name="--workspace" type="boolean">
  Scaffold a monorepo-style codemod workspace. The root keeps shared repo and publishing files, and new codemods live under `codemods/<slug>/`.

  In interactive mode, this changes the generated layout to a dedicated codemods repository structure. In `--no-interactive` mode, pair it with the values normally gathered from prompts, such as `--name`, `--description`, `--author`, `--license`, and `--package-manager`.
