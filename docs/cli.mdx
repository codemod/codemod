...
```bash
npx codemod jssg run <codemod_file> [--target <target_directory>] [options]
```
...
<ResponseField name="--target <TARGET_DIRECTORY>, -t <TARGET_DIRECTORY>" type="string">
  Directory to apply the codemod to. Defaults to the current working directory when omitted.
</ResponseField>
...
<Accordion title="Examples">
  ...
  npx codemod jssg run ./scripts/codemod.ts --target ./src --language tsx

  # Preview changes without writing files
  npx codemod jssg run ./scripts/codemod.ts --target ./src --language tsx --dry-run
  ```
</Accordion>
