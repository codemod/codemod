...
This is the command that harness integrations invoke after `codemod ai` installs a Codemod MCP entry.
...
<Info>
  `codemod mcp` is intended to be launched by an MCP client or harness configuration, not used as an interactive terminal command.
</Info>
