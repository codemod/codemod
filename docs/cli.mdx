...
Install the `codemod` MCS skill package and configure Codemod MCP.

```bash
npx codemod agent install [--harness <claude|goose|opencode|cursor|codex|antigravity|auto>] [--project|--user] [--force] [--no-interactive] [--update-policy <manual|notify|auto-safe>] [--update-source <local|registry|URL>] [--require-signed-manifest|--allow-unsigned-manifest] [--format <logs|table|json|yaml>]
```
...
- `antigravity`: not configured yet (skill install only)

If a conflicting `mcpServers.codemod` entry already exists, re-run with `--force` to overwrite it.

`agent install` also upserts a managed Codemod discovery section in harness discovery guides (project root for `--project`, home directory for `--user`) without duplicating entries:
- `claude`: `CLAUDE.md`
- `goose`, `opencode`, `cursor`: `AGENTS.md` and `CLAUDE.md`
- `codex`, `antigravity`: `AGENTS.md`
`agent install` also wires a periodic, cooldown-protected update check trigger:
...
- `goose`: managed `codemod-periodic-update` block in `./.goosehints` (project) or `~/.goosehints` (user) as fallback guidance
- `codex`: `notify = ["sh", "<runner>"]` in `./.codex/config.toml` (project) or `~/.codex/config.toml` (user)
- `antigravity`: managed guidance in `./.agents/CODEMOD_PERIODIC_UPDATE.md` (project) or `~/.gemini/antigravity/CODEMOD_PERIODIC_UPDATE.md` (user)
...
After install, restart or reload your harness session so newly installed skills are picked up.
...
```bash
npx codemod agent update [--harness <claude|goose|opencode|cursor|codex|antigravity|auto>] [--project|--user] [--force] [--no-interactive] [--update-policy <manual|notify|auto-safe>] [--update-source <local|registry|URL>] [--require-signed-manifest|--allow-unsigned-manifest] [--format <logs|table|json|yaml>]
```

If managed install state is missing, `agent update` automatically falls back to an install flow and then applies update reconciliation.

**`agent list`**

List installed codemod-managed skills (MCS and package skills).

```bash
npx codemod agent list [--harness <claude|goose|opencode|cursor|codex|antigravity|auto>] [--format <logs|table|json|yaml>]
```

`agent install` performs internal skill validation checks during install flow.
...
This flow also updates managed Codemod discovery hints in harness discovery guides (`CLAUDE.md` for Claude, `AGENTS.md` and `CLAUDE.md` for Goose/OpenCode/Cursor, `AGENTS.md` for Codex/Antigravity) without duplication, then prompts you to restart/reload the harness session.
...
<Info>
  **TUI auto-launch**: On Unix systems (macOS, Linux), if the workflow contains [manual nodes or manual triggers](/workflows/reference#manual-triggers), an interactive TUI dashboard launches automatically so you can monitor progress and trigger tasks in real time. Use `--no-interactive` to disable this behavior.
</Info>
...
Launch an interactive terminal dashboard to browse workflow runs and manage tasks. Available on Unix systems (macOS, Linux) only.

```bash
npx codemod workflow tui
```

<ResponseField name="-i, --id <ID>" type="string">
  Go directly to the task list for a specific workflow run.
</ResponseField>

<ResponseField name="-l, --limit <LIMIT>" type="number">
  Maximum number of workflow runs to display (default: 20).
</ResponseField>

The TUI provides two main views:

- **Run list**: Shows recent workflow runs with status, name, timing, and duration.
- **Task list**: Shows individual tasks within a run with their status and logs.
...
| `Enter` or `l` | View task logs |
| `t` | Trigger a single awaiting task |
| `T` | Trigger all awaiting tasks |
| `R` | Retry a failed task |
| `c` | Cancel the workflow |
| `s` | Open settings |
| `Esc` | Go back |
| `q` | Quit |

<Info>
  The TUI also launches automatically when running `workflow run` on a workflow that contains [manual nodes or triggers](/workflows/reference#manual-triggers). Use `--no-interactive` to keep the text-based interface instead.
</Info>
...
<ResponseField name="--filter" type="string">
  A pattern to run only tests whose names match the filter. Test names correspond to the subdirectory names within the test directory.
</ResponseField>
