...
Start the Codemod MCP server over stdio for editor and agent integrations.

```bash
npx codemod mcp [--usage-log <PATH>] [--allow-feedback]
```

- Use this command when you are wiring Codemod into an MCP client manually.
- `--usage-log <PATH>` writes MCP usage events to a local file for debugging.
- `--allow-feedback` records this OS user's consent for short anonymous Codemod AI and MCP feedback submissions. This is the same consent used by `npx codemod ai feedback`.

```bash
# Start the MCP server directly
npx codemod mcp

# Capture MCP usage events while debugging a client setup
