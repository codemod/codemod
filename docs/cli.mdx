...
<Info>
  `codemod agent` and `codemod tcs` are currently in an early scaffold phase for AI-native CLI work. Command parsing/help is available, but full install/list/verify/run behavior is still being implemented.
</Info>
