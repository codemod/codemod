...
### Direct package skill install (`--skill`)
...
```bash
npx codemod <package-id> --skill [--harness <claude|goose|opencode|cursor|codex|antigravity|auto>] [--project|--user] [--force] [--no-interactive] [--format <logs|table|json|yaml>]
```

Direct skill install also runs interactively by default in TTY sessions; pass `--no-interactive` for non-interactive execution.

<ResponseField name="--harness <VALUE>" type="string">
  Target harness skill root. `auto` resolves by workspace marker order: `.claude`, `.goose`, `.opencode`, `.cursor`, `.codex`, `.agents`, then Claude fallback.
</ResponseField>
...
<ResponseField name="--format <logs|table|json|yaml>" type="string">
  Output contract format. Default: `logs` (human-readable).
</ResponseField>

Without `--skill`, package invocation follows normal `codemod run` behavior.
