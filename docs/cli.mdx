...
Use this when you want an MCP-compatible client or harness to talk to Codemod tools and resources through a locally launched CLI process.
...
<Info>
  `codemod ai` configures supported harnesses to launch this server automatically, usually through `npx codemod@latest mcp`. Run `codemod mcp` directly when you need to test or debug the MCP server outside harness-managed setup.
</Info>
