...
```bash
npx codemod ai [--allow-feedback] [--harness <claude|goose|opencode|cursor|codex|auto>] [--project|--user] [--force] [--no-interactive] [--update-policy <manual|notify|auto-safe>] [--update-source <local|registry|URL>] [--require-signed-manifest|--allow-unsigned-manifest] [--format <logs|table|json|yaml>]
```
...
Use `--allow-feedback` to record anonymous Codemod AI and MCP feedback consent for the current OS user during install. The CLI stores that consent locally and reuses it for later `ai feedback` and MCP feedback-capable flows.
...
```bash
npx codemod ai update [--allow-feedback] [--harness <claude|goose|opencode|cursor|codex|auto>] [--project|--user] [--force] [--no-interactive] [--update-policy <manual|notify|auto-safe>] [--update-source <local|registry|URL>] [--require-signed-manifest|--allow-unsigned-manifest] [--format <logs|table|json|yaml>]
```
...
Start the Codemod MCP server over stdio.
...
- `--allow-feedback` records anonymous Codemod AI and MCP feedback consent for the current OS user before the server starts.
