...
Codemod stores cached packages by package name and version. Scoped packages are cached under their scope, and unscoped packages are cached under a shared `global/` directory inside the cache root.
...
Prints the cache location, total cached packages, total cached versions, and aggregate disk usage.
...
Without `--detailed`, the command prints one row per cached package with the number of cached versions and total size. With `--detailed`, each row also includes per-version size and how long ago that version was last modified.
...
Use `PACKAGE` to remove all cached versions for a single package. Use `--all` to remove the entire local package cache and recreate an empty cache directory.

<ResponseField name="[PACKAGE]" type="string">
  Package name (for example `@org/package` or `package`).
</ResponseField>
...
Prunes cached package versions older than `--max-age` days.
...
<ResponseField name="--dry-run" type="boolean">
  Show which cached package versions would be removed without deleting them.
</ResponseField>
