...
Start the Codemod Model Context Protocol (MCP) server over stdio.

```bash
npx codemod mcp [--usage-log <PATH>]
```

Use this command when you want an MCP-compatible client or harness to talk to Codemod through the CLI.
...
<Info>
  This command is primarily intended to be launched by MCP clients and Codemod-managed AI integrations, not run directly in a normal terminal session.
</Info>
