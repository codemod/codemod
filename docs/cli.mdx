...
- `claude`: `CLAUDE.md`
- `goose`, `cursor`: `AGENTS.md` and `CLAUDE.md`
- `opencode`: `AGENTS.md`
- `codex`: `AGENTS.md`
...
This flow also updates managed Codemod discovery hints in harness discovery guides (`CLAUDE.md` for Claude, `AGENTS.md` and `CLAUDE.md` for Goose/Cursor, `AGENTS.md` for OpenCode/Codex) without duplication, then prompts you to restart/reload the harness session.
...
In interactive mode, `codemod init` prompts for the codemod name and author, then shows a review screen before scaffolding. You can edit all other fields from that review screen before creating files.

Interactive defaults:

- directory: name-derived package directory when `[PATH]` is omitted
- codemod type: `ast-grep-js` with agent skill behavior
- agent skill behavior: included by default for workflow packages
- target language: `typescript`
- description: `Codemod package for <name>`
- author: prompted; defaults to the logged-in user as `username <email>` when available
- private package: `false`
- license: `MIT`
- package manager: `npm`
- GitHub Actions publishing workflow: disabled

You can enable a GitHub Actions publishing workflow from the review screen. This generates `.github/workflows/publish.yml`:
...
<ResponseField name="[PATH]" type="string">
  Project directory name. In interactive mode, when omitted, the CLI defaults to a directory derived from the final codemod name.
</ResponseField>

<ResponseField name="--name <NAME>" type="string">
  Project name (defaults to directory name).

  Required in `--no-interactive` mode only when the project name cannot be derived from `[PATH]`.
</ResponseField>
...
<ResponseField name="--project-type <PROJECT_TYPE>" type="string">
  ...
  Defaults to `ast-grep-js` in `--no-interactive` mode for workflow packages.
...
<ResponseField name="--package-manager <PACKAGE_MANAGER>" type="string">
  ...
  Defaults to `npm` in `--no-interactive` mode for `ast-grep-js`, `hybrid`, and `--workspace`.
</ResponseField>

<ResponseField name="--language <LANGUAGE>" type="string">
  Target language.

  Defaults to `typescript` in `--no-interactive` mode.
</ResponseField>

<ResponseField name="--description <DESCRIPTION>" type="string">
  Project description.

  Defaults to `Codemod package for <name>`.
</ResponseField>

<ResponseField name="--author <AUTHOR>" type="string">
  Author name and email stored in `codemod.yaml` package metadata.

  Interactive mode prompts for this value and defaults to the logged-in user as `username <email>` when available.
  In `--no-interactive` mode, the CLI uses `--author`, then the logged-in user, then the OS username.
</ResponseField>
...
<Info>
  ...
  `author` is required in `codemod.yaml`.
</Info>
