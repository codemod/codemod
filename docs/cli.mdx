...
- author: logged-in user as `username <email>` when available; otherwise omitted
- license: `MIT`
- package manager: `npm`
...
<ResponseField name="--author <AUTHOR>" type="string">
  ...
  Defaults to the logged-in user as `username <email>` when available. Otherwise omitted.
</ResponseField>
...
<Info>
  ...
  If `author` is omitted locally, `codemod publish` resolves it from the authenticated user before upload.
</Info>
