...
In interactive mode, `codemod init` prompts for the core package decisions, then shows a review screen before scaffolding. You can edit fields from that review screen before creating files.
...
<ResponseField name="--author <AUTHOR>" type="string">
  ...
  Defaults to `Author <author@example.com>`.
</ResponseField>
...
<Info>
  ...
  If `author` is missing or still the placeholder value, `codemod publish` warns and can replace the uploaded package's `codemod.yaml` author metadata with values inferred from the authenticated user. This does not affect the Registry Publisher shown in Codemod Registry, which always reflects the logged-in publishing account.
</Info>
