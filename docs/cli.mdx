...
Use this command when you are wiring Codemod into an MCP client manually or when you want to debug the MCP server outside the guided `codemod ai` install flow.
...
<Info>
  In most setups, you should run `npx codemod ai` instead of configuring `codemod mcp` directly. The `ai` command installs the MCP server entry for supported harnesses and also installs the related Codemod skill files and command entrypoints.
</Info>
