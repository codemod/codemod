...
<ResponseField name="--description <DESCRIPTION>" type="string">
  ...
  Defaults to `Codemod package for <name>` in `--no-interactive` mode.
</ResponseField>

<ResponseField name="--author <AUTHOR>" type="string">
  Author name and email.

  Defaults to the logged-in user as `username <email>` in `--no-interactive` mode when available. Otherwise omitted.
</ResponseField>

<ResponseField name="--license <LICENSE>" type="string">
  License.

  Defaults to `MIT` in `--no-interactive` mode.
</ResponseField>
