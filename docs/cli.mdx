...
```bash
npx codemod agent install [--harness <claude|goose|opencode|cursor|auto>] [--project|--user] [--force] [--interactive] [--format <table|json|yaml>]
```
...
If the target path is a skill-only package (`agents/skill/<skill-name>/SKILL.md` without `workflow.yaml`), validation returns actionable guidance to use direct install mode:

```bash
npx codemod <package-id> --skill
```
...
<ResponseField name="--skill" type="boolean">
  Scaffold a skill-only package (no `workflow.yaml`) with authored files under `agents/skill/<skill-name>/`.
</ResponseField>

<ResponseField name="--with-skill" type="boolean">
  Scaffold a hybrid package (workflow + skill). This keeps workflow files and also creates authored skill files under `agents/skill/<skill-name>/`.
</ResponseField>
