...
This command is usually launched by your MCP client configuration rather than run directly in a terminal.
...
<ResponseField name="--allow-feedback" type="boolean">
  Opt in this OS user to anonymous Codemod AI and MCP feedback submission before the server starts.
</ResponseField>

<Info>
  `codemod ai` installs MCP client entries that typically run `npx codemod@latest mcp` for you. Use `codemod mcp` directly when you are wiring a custom MCP client or debugging a local integration.
</Info>
