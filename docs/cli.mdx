...
<Info>
  `codemod login` also enables authenticated actions from the local report UI opened by
  `codemod run --report` and `codemod workflow run --report`. If you are not logged in, the Share
  dialog asks you to authenticate before it can upload a report link.
</Info>
