...
```bash
npx codemod agent install [--harness <claude|goose|opencode|cursor|auto>] [--project|--user] [--force] [--interactive] [--periodic-policy <manual|notify|auto-safe>] [--update-policy <manual|notify|auto-safe>] [--update-source <local|registry|URL>] [--require-signed-manifest|--allow-unsigned-manifest] [--format <table|json|yaml>]
```
...
`agent install` also upserts a managed Codemod discovery section in `AGENTS.md` and `CLAUDE.md` (project root for `--project`, home directory for `--user`) without duplicating entries.
...
Periodic update policy is install-flag driven:
- `--periodic-policy <manual|notify|auto-safe>` (default: `auto-safe`)
Install-time managed update policy is also flag-driven:
- `--update-policy <manual|notify|auto-safe>` (default: `manual`)
...
```bash
npx codemod agent list [--harness <claude|goose|opencode|cursor|auto>] [--format <table|json|yaml>]
```
...
This flow also updates managed Codemod discovery hints in `AGENTS.md` and `CLAUDE.md` without duplication, then prompts you to restart/reload the harness session.

```bash
npx codemod <package-id> --skill [--harness <claude|goose|opencode|cursor|auto>] [--project|--user] [--force] [--format <table|json|yaml>]
```
