...
<Info>
  AI-native CLI commands are in active rollout. `codemod agent install-skills|list-skills|verify-skills`, `codemod tcs install`, and `codemod tcs inspect` are available; `codemod agent run` and `codemod tcs run` remain in scaffold phase.
</Info>
