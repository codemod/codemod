...
<Info>
  Bare `npx codemod` opens an interactive launcher. In npm-based usage, that launcher refreshes to the latest published CLI before prompting you to create a package or run one from the registry.
</Info>
