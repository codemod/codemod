...
Start the Codemod MCP server over stdio for editors and coding agents that support the Model Context Protocol.
...
- `--allow-feedback` records consent for this OS user so Codemod MCP can submit short anonymous platform feedback.

Use this command when you want to wire Codemod into an MCP client manually instead of using `npx codemod ai` to bootstrap the integration.
