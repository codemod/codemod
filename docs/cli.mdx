...
```
**Use for**: Opening the interactive launcher, running production codemods, multi-step workflows, parameterized transforms

In interactive terminals, bare `npx codemod` opens a launcher with shortcuts for installing Codemod AI integrations, creating a new package, or running a published package. In non-interactive contexts, it prints next steps and exits with status `1`.
