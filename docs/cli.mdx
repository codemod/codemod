...
Global option:
- `--disable-telemetry`: disable CLI telemetry for the current invocation, including package runs started with `npx codemod <package-name>` and local runs through `npx codemod workflow run ...`.
