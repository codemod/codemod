...
<Info>
  In an interactive terminal, bare `npx codemod` opens a launcher with three next steps: install Codemod AI, create a new package, or run a published package from the registry. In non-interactive contexts, the CLI prints those same next steps and exits with status `1` instead of prompting.
</Info>
