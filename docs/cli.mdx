...
Validate a workflow file or codemod package.

```bash
npx codemod workflow validate -w <workflow.yaml|package-directory>
```
...
`workflow validate` also accepts nested paths inside a package and resolves the nearest ancestor containing `codemod.yaml`. If you pass a directory that is not a package, the CLI validates `workflow.yaml` inside that directory.

Passing a standalone `SKILL.md` path is not supported because skill validation requires the surrounding package context.
