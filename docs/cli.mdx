...
<ResponseField name="--format <FORMAT>" type="string">
  ...
  In `json` and `yaml` formats, the response also includes:
  - `metadata_coverage`: summary of how completely matching packages fill in key registry metadata
  - `threshold_route_preview`: Codemod's suitability routing preview for the current result set
  - `packages[].metadata_coverage`: per-package metadata coverage details
</ResponseField>
...
<Accordion title="Examples">
  ...
  Preview package metadata completeness and routing signals in YAML:

  ```bash
  npx codemod search react --format yaml
  ```
</Accordion>
