...
When you run a codemod with `--report`, the local report UI also exposes a **Feedback** button. You can submit feedback manually or choose **Auto-submit with local agent** to let an installed coding agent inspect the target read-only, draft anonymous feedback from the run summary, and submit it for you. Feedback is disabled when `DISABLE_ANALYTICS` is set.
...
<ResponseField name="--report" type="boolean">
  Open the local execution report in your browser after the run completes. The report UI shows metrics, changed files, and diffs. You can also submit anonymous codemod performance feedback from the report, either manually or by letting a supported local coding agent draft it from the run context.
</ResponseField>
