...
Start the Codemod MCP (Model Context Protocol) server over stdio.

```bash
npx codemod mcp [options]
```

Use this command when you want to wire Codemod into an MCP-compatible client manually. Most users should prefer `npx codemod ai`, which installs the MCP entry and related Codemod skill files for supported harnesses.

<ResponseField name="--usage-log <PATH>" type="string">
  Write MCP usage events to a local file for debugging.
</ResponseField>

<ResponseField name="--allow-feedback" type="boolean">
  Opt in this OS user to anonymous Codemod AI and MCP feedback before starting the server.

  This records feedback consent in the user-wide Codemod config. It does not submit feedback by itself.
</ResponseField>
...
<Accordion title="Example">
  ...
  # Start the server and log usage events for debugging
  npx codemod mcp --usage-log ./codemod-mcp-usage.log
  ```
</Accordion>
