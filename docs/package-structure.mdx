...
- **CLI:** `codemod run @scope/pkg --workflow <name>` selects a variant. Without `--workflow`, the entry marked `default: true` runs (or the first entry if none is flagged). When stdin is interactive and no flag is given, the CLI shows a picker.
...
**Naming rules:** Use either `package-name` or `@scope/package-name`.

Allowed characters are lowercase letters, numbers, hyphens, and underscores. A single `/` is allowed only to separate a scope from the package name.

**Not allowed:** uppercase letters, dots, commas, spaces, multiple `/` separators, or other special characters
...
- <code>@scope/pkg/extra</code>
- <code>my codemod</code>
</ParamField>
...
- `name` (required) — package-local identifier matching `^[A-Za-z0-9_-]+$`. Used by `codemod run --workflow <name>` and the cloud campaign picker.
