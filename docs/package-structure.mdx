...
For workflow scaffolds, `codemod init` can now generate starter codemods for these target languages:
...
These language selections update the generated starter script, fixture files, and YAML ast-grep config templates to match the chosen target.
