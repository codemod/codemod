...
The scaffolded script and fixture extensions depend on the selected target language. For example, `codemod init --target-language xml` generates XML-oriented starter files such as `scripts/codemod.xml.ts`, `rules/config.xml.yml`, and `tests/.../input.xml`.
