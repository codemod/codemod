...
| `scripts/` | JavaScript/TypeScript codemods ([JSSG](/jssg/quickstart)) |
| `rules/` | YAML ast-grep rule definitions |
| `tests/` | Test fixtures with input/expected pairs |
...
Codemod packages can provide:
- **Workflow** (`workflow.yaml`) for `codemod run` / `codemod workflow run`
- **Skill** (`agents/skill/<skill-name>/SKILL.md` + references) installable with `npx codemod <package-id> --skill`
- **Hybrid** (both workflow + skill)

Current CLI behavior:
- `codemod init` (default) scaffolds workflow-only packages.
- `codemod init --skill` scaffolds skill-only packages.
- `codemod init --with-skill` scaffolds hybrid packages.
- `codemod publish` supports workflow-only, skill-only, and hybrid packages (at least one `provides` value must be present).
- `codemod workflow validate -w <package-root>` validates declared package behaviors:
  - workflow-only: workflow checks
  - skill-only: skill structure/frontmatter/reference checks
  - hybrid: both workflow and skill checks
...
All files in this authored skill directory are treated as install payload and copied recursively during `npx codemod <package-id> --skill`.
...
For skill-only packages, omit `workflow`, include `provides: ["skill"]`, and author skill content under `agents/skill/<skill-name>/`.
...
Relative path to your workflow file (required for workflow-capable packages, optional for skill-only packages).
</ParamField>
...
<ParamField path="provides" type="string[]" default='["workflow"]'>
Array of package behavior tags.

- `workflow` - Package provides executable workflow support (`workflow.yaml`)
- `skill` - Package provides installable skill support (`agents/skill/<skill-name>/SKILL.md`)
</ParamField>

<ParamField path="capabilities" type="string[]">
Optional runtime permission tags for JSSG execution.
...
<Warning>
Only enable capabilities your codemod actually needs. Runtime permission capabilities increase security risk for untrusted codemods.
</Warning>
