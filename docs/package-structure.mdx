...
For nested codemods, these package-level capabilities are additive. If package `A` runs package `B`, Codemod evaluates the child package's `capabilities` together with any capabilities already granted by the parent package or CLI flags.
