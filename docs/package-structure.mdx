...
- **Hybrid**: `workflow.yaml` has both executable and `install-skill` steps, with authored skill files under `agents/skill/<skill-name>/`.
...
- `codemod publish` supports workflow-only, skill-only, and hybrid packages by validating workflow behavior and skill layout.
...
Relative path to your workflow file (required for workflow-only, skill-only, and hybrid packages).
</ParamField>
