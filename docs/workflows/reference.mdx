...
<Warning>
Shell command steps are a privileged escape hatch. Depending on the run surface, Codemod may require explicit approval before executing a `run` step.
</Warning>
...
<Info>
During interactive workflow runs, Codemod can show the fully resolved shell command and require confirmation before it executes. This applies after variable interpolation, so review the exact command that will run.
</Info>
