...
To run a shell script that ships with your codemod package, reference it from `$CODEMOD_PATH`, which points to the package root:

```yaml workflow.yaml
steps:
  - name: Install dependency
    run: $CODEMOD_PATH/scripts/add-dependency.sh
```
...
<ParamField path="run" type="string" required>
  Shell command to execute. Use `$CODEMOD_PATH` for scripts or files bundled with the codemod package.
</ParamField>
