...
<ParamField path="bump-dependency.manager" type="string">
  Package manager override. Supported managers include `npm`, `yarn`, `pnpm`, `bun`, `cargo`, `go`, `pip`, `poetry`, `pipenv`, `bundler`, `maven`, and `gradle`. If omitted, Codemod infers the manager from package files in the repository.
</ParamField>
