...
<ParamField path="ai.max_output_tokens" type="number" default="8192">
  Maximum output tokens per model response. Resolution order: `ai.max_output_tokens` -> `LLM_MAX_OUTPUT_TOKENS` -> `8192`.
</ParamField>
...
<ParamField path="ai.api_key" type="string">
  ...
  LLM_MODEL=gpt-4o
  LLM_MAX_OUTPUT_TOKENS=8192  # used when ai.max_output_tokens is omitted
