...
<ParamField path="bump-dependency.dependencies[].name" type="string" required>
  Package name as it appears in the package manager manifest, for example `react`, `@scope/package`, `github.com/gin-gonic/gin`, or `serde`.
</ParamField>
...
<ParamField path="bump-dependency.manager" type="string">
  Package manager override. Supported command-based managers include `npm`, `yarn`, `pnpm`, `bun`, `cargo`, `go`, `pip`, `poetry`, `pipenv`, and `bundler`. If omitted, Codemod infers the manager from package files in the repository.
</ParamField>
...
<Info>
In dry-run mode, Codemod plans and logs the package-manager command without changing files.
</Info>

<Warning>
Maven and Gradle projects are detected, but Java dependency updates are not executed by this step yet.
</Warning>
