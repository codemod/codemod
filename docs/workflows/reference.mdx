...
`ensure` describes the desired dependency range. If the current declaration already satisfies the range, the dependency is skipped. If it does not, Codemod writes the `ensure` range as the target version.
...
<ParamField path="bump-dependency.dependencies[].ensure" type="string">
  Desired version range. If the current declaration already satisfies this range, Codemod skips the dependency. If it does not, Codemod updates the dependency to this range unless `target` is provided.
</ParamField>

<ParamField path="bump-dependency.dependencies[].if_version" type="string">
  Conditional match range. Use this when you only want to update dependencies currently matching a specific range. `*` and `any` match any existing version. Requires `target`.
</ParamField>

<ParamField path="bump-dependency.dependencies[].target" type="string">
  Version range to write. Required with `if_version`. Optional with `ensure`; when omitted, Codemod writes the `ensure` range. Target syntax is package-manager-specific.
</ParamField>
