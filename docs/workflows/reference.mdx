...
<ParamField path="install-skill.harness" type="string" default="auto">
  Target harness adapter: `auto`, `claude`, `goose`, `opencode`, `cursor`, `codex`, or `antigravity`.
</ParamField>
...
Expression interpolation is currently used for shell commands, AI prompts, commit messages, branch names, and pull request title/body. The runtime support is narrower than GitHub Actions style templating, so only the sources below are wired into workflow expression resolution today:

| Syntax | Source | Example |
|--------|--------|---------|
| `${{ params.x }}` | Runtime parameters | `${{ params.branch }}` |
| `${{ state.x }}` | Shared state | `${{ state.counter }}` |
| `${{ matrix.x }}` | Matrix item values | `${{ matrix.team }}` |
| `${{ task.id }}` | Current task UUID | Available in `branch_name`, `commit.message`, and `pull_request.title`/`body` during Campaign/cloud runs |
| `${{ task.signature }}` | Short deterministic task hash | Available in the same git automation fields; default branch names use this value |
...
<Info>
`task.*` variables are currently only wired into git automation fields used during Campaign/cloud execution. `${{ env.x }}` and generic `steps.<id>.outputs.*` interpolation are not currently wired into workflow expression resolution.
</Info>
