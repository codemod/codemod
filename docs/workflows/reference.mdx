...
<Info>
Shell steps inherit the parent process environment by default, in addition to any step-level `env`
values. This includes inherited LLM settings such as `LLM_API_KEY` and `LLM_BASE_URL` when you run
the workflow from an environment that already defines them.
</Info>
