...
<Tip>
  This is an advanced topic. If you're just getting started, we recommend using [Codemod Studio](/platform/codemod-studio) or [Codemod MCP](/model-context-protocol) to create your Codemod package with the help of AI.
</Tip>
...
<Step title="Running workflows">
  ...
  ```bash Running a package
  npx codemod workflow run -w ./my-codemod-package/
  ```

  ```bash Directly running a workflow file
...
Expand commentComment on line R224Resolved
</Tip>
</ParamField>
...
- [`js-ast-grep`](#jssg-step) - **Primary transformation engine**: JavaScript/TypeScript codemods using ast-grep
- [`ast-grep`](#yaml-ast-grep-step) - Declarative pattern matching with ast-grep YAML rules
- [`run`](#shell-command-step) - Run shell commands (e.g. setup, cleanup, invoking external tools)
- [`ai`](#ai-step) - LLM-powered transformations and tasks (e.g. non-deterministic transformations, code reviews, summarization)
- [`codemod`](#codemod-registry-step) - Run codemods from the Codemod Registry

#### JSSG step

Executes a JavaScript/TypeScript codemod using ast-grep for pattern matching and AST manipulation. JSSG is the primary transformation engine for Codemod, combining the speed of ast-grep with the flexibility of JavaScript.

JSSG steps are useful for:

- Complex transformations requiring conditional logic or loops
- Cross-file coordination or context-aware changes
- Custom AST manipulations beyond simple find/replace
...
<Steps>
  <Step title="Create a JSSG codemod">
    ...
    Learn more: [JSSG Quickstart](/jssg/quickstart) and [API Reference](/jssg/reference).
  </Step>
...
<Steps>
  <Step title="Find or publish a codemod to the Codemod Registry:">
    ...
    - [publish a Codemod package](/cli#codemod-publish) to Codemod Registry.
  </Step>
...
<Info>
**Enterprise Features:** In the Codemod app, you can configure parameters visually through the UI. See [Campaigns](/platform/campaigns) for running parameterized workflows at scale.
</Info>
...
<CardGroup cols={2}>
  <Card title="Codemod CLI Reference" icon="terminal" href="/cli">
