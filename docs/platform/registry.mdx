...
# Inspect registry metadata coverage in machine-readable output
npx codemod search react --format json
```

When you use `--format json` or `--format yaml`, the CLI also returns `metadata_coverage` and `threshold_route_preview` summaries so you can audit package discoverability quality or feed the results into automation.
