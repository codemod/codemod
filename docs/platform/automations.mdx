...
The **Scan Codebase** action uses the same query model as [Insights](/platform/insights). It can run ast-grep rules, JSSG queries, or [Smart](/platform/insights#smart-queries) natural-language file checks and return either:

- A scalar result available as `scan.totalCount`
- Table rows available as `scan.rows`

Smart scans always return file counts. Use scan output in later templates to decide whether to create a campaign, send a webhook, or ask a coding agent to work only when the query finds matching code.
