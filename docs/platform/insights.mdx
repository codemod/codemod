...
The dashboard-level time range controls the default range for timeseries widgets. New dashboards use a 14-day default range, and each timeseries widget can either inherit that range or override it.

<Tip>
  When you are editing an Insights dashboard, Ask Codemod Wish can inspect the current dashboard draft and add or update widgets. Review the changes and save the dashboard when the draft looks right.
</Tip>

## Widgets

Widgets are the building blocks of a dashboard. Query-backed widgets run one or more queries against your repositories and render the results in one of three formats:
...
You can also add a **Notes** widget for context, caveats, interpretation, or next actions. Notes widgets use Markdown and do not run queries.
