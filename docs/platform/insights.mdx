...
- Exact syntax matches → [ast-grep YAML](#ast-grep-yaml-queries)
- Custom metrics or programmatic analysis → [JSSG](#jssg-codemod-queries)
- Package inventory → [Dependencies](#dependency-queries)

Smart is an Insights widget over indexed repository snapshots. It is not Grep search and does not return line matches or code snippets.
...
<Tip>
  Codemod Wish can help add or edit widgets on a dashboard. Smart widgets follow the live dashboard schema (`type: "smart"`). There is no separate CLI or MCP command for Smart queries.
</Tip>
