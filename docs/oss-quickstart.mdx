...
If you already know the language you want to target, pass it during scaffolding so the starter script, tests, and sample workflow line up with that parser. For example, XML codemods can start with `npx codemod init --target-language xml`.
