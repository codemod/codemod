...
In an interactive terminal, `codemod init` lets you choose between a `jssg codemod`, `multi-step workflow`, or `agent skill codemod`. If you choose a workflow package, you can also scaffold an authored agent skill alongside it.
...
If you already know which package shape you want, you can scaffold it directly:

```bash
# Workflow package
npx codemod init --project-type ast-grep-js

# Skill-only package
npx codemod init --skill

# Workflow package with an authored skill
npx codemod init --project-type ast-grep-js --with-skill
```

Use a workflow package when you want to run transforms. Use a skill package when you want to distribute authored guidance and agent behavior. Use a combined package when you need both.
