...
<Tip>
  `codemod init` can also set up a **monorepo** to host multiple codemods with shared GitHub Actions for automated publishing. See [CLI reference](/cli#codemod-init) for all available options.
</Tip>

## 2. Install Codemod MCP

Codemod MCP gives your AI assistant tools for AST analysis, codemod creation, and testing, directly in your IDE.
...
Tests use input/expected file pairs in the `tests/` directory. If MCP generated test fixtures, they will run automatically.
...
<Tip>
  **AI-powered steps**: If your workflow includes an AI step gated by a parameter, enable it at runtime:
  ...
  Set `LLM_API_KEY` as an environment variable for your chosen provider. See [Workflow Reference — AI steps](/workflows/reference#ai-step) for provider configuration.
</Tip>
