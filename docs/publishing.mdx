...
When you run `npx codemod publish`, the CLI validates the package before it uploads the tarball:

- It checks that `codemod.yaml` exists and parses correctly.
- It validates the package structure for workflow packages, skill packages, or packages that include both.
- It validates every referenced workflow file.
- It bundles JavaScript dependencies referenced by `js-ast-grep` workflow steps.
- It stops if a referenced transform imports a module that cannot be resolved.

Use this as a packaging check, not just an upload command. If local validation fails, nothing is published.
...
<Warning>
  `codemod publish` fails before upload when a referenced transform imports a module that cannot be resolved or bundled. Runtime-provided modules such as Node built-ins and `codemod:` modules are allowed, but package-local dependencies must be present and resolvable at publish time.
</Warning>

If publish fails with an unresolved import error:

- Install the missing dependency in the package you are publishing.
- Check that the import path is correct relative to the package root.
- Keep runtime externals limited to supported runtime modules.
- Re-run `npx codemod publish` after the package bundles successfully.
