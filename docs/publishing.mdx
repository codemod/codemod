...
| [API Keys](#api-keys) | CI/CD pipelines, automation | Yes (`CODEMOD_API_KEY` or `CODEMOD_AUTH_TOKEN`) | Create key in UI |
...
If you already have an API key, you can also store it through the CLI instead of using the browser flow:
...
API keys are a good fit for CI systems that cannot use GitHub Actions OIDC or for local automation where you do not want to open a browser.

The CLI supports two API-key based publish patterns:

- `npx codemod login --api-key ...` to validate and persist the API key as your stored auth for later `codemod publish` runs
- `CODEMOD_AUTH_TOKEN=... npx codemod publish` to inject a token for a single publish without writing auth to disk

`CODEMOD_API_KEY` is still a convenient environment variable name for your own scripts or CI secrets, but `codemod publish` reads `CODEMOD_AUTH_TOKEN` directly when deciding which token to use.
