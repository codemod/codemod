...
# Log out from every configured registry and clear cached packages
npx codemod logout --all
```
...
- `npx codemod whoami` shows the currently authenticated user for the default registry.
- `npx codemod whoami --detailed` also shows token type, scopes, refresh-token availability, and expiry when present.
- `npx codemod logout` removes the saved auth session for the default registry.
- `npx codemod logout --all` signs you out of every configured registry and clears the local package cache.
