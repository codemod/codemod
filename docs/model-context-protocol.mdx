...
If you want to run the MCP server directly, use:

```bash
npx codemod mcp
```

For debugging MCP tool usage locally, the server also supports writing usage events to a file:
...
<AccordionGroup>
  <Accordion title="Server won't start">
    ...
    - For local debugging, run `npx codemod mcp --usage-log ./codemod-mcp-usage.log` and inspect the recorded MCP usage events
  </Accordion>
