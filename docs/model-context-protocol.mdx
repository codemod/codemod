...
Codemod exposes a small set of MCP tools directly through the MCP server, and mirrors the same surfaces through `npx codemod ai` for harnesses or environments where direct MCP access is unavailable.
...
CLI mirrors for inspection and debugging:

- **`npx codemod ai dump-ast`** - Inspect AST output for a file or stdin
- **`npx codemod ai node-types`** - Print supported tree-sitter node types for a language
- **`npx codemod ai tools`** / **`npx codemod ai tool <name>`** - List tool names or inspect one tool's JSON schema
- **`npx codemod ai call <name>`** - Call an MCP-equivalent tool directly with JSON input
- **`npx codemod ai resources`** / **`npx codemod ai resource <uri-or-name>`** - List and read MCP-equivalent docs resources from the CLI
- **`npx codemod ai docs [query...]`** - List, read, or search docs-backed Codemod resources from the CLI

<Tip>
  Use the CLI mirrors when you need to inspect available tools and resources from a terminal, validate JSON input schemas before wiring an agent, or troubleshoot a harness setup without opening the MCP client directly.
</Tip>
...
- **`jssg-utils-instructions`** - Docs-backed JSSG utility guidance, including JavaScript imports, Java helpers, and XML helpers
