...
Most users should use `npx codemod ai`, which installs and configures Codemod MCP for a supported harness. If you need to launch the MCP server yourself, run:

```bash
npx codemod mcp
```

To capture MCP usage events while debugging an integration, pass `--usage-log` with a file path:

```bash
npx codemod mcp --usage-log ./codemod-mcp-usage.log
```

This writes MCP usage events to the provided file while the server is running over stdio.
