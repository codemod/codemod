...
If you need to launch the server directly, the underlying command is:

```bash
npx codemod mcp
```

For debugging MCP client integrations, you can also log usage events to a file:

```bash
npx codemod mcp --usage-log ./codemod-mcp.log
```
