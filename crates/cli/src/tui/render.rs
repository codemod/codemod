...
use super::app::App;
use super::pty::{resize_pty_and_parser, resize_slots};
...
pub fn render_breadcrumb(f: &mut Frame, app: &App, area: Rect) {
    ...
    if app.screen == Screen::Actions || app.screen == Screen::Terminal {
        ...
        let task_name = if app.screen == Screen::Terminal {
            if app.terminal_mode == TerminalMode::Multi {
...
pub fn render_tasks_screen(f: &mut Frame, app: &mut App, area: Rect, elapsed_ms: u128) {
    ...
    let preview_content: Vec<Line> = if let Some(idx) = app.tasks_state.selected() {
        if let Some(task) = app.tasks.get(idx) {
            ...
            if !app.terminal_entries.is_empty() {
                lines.push(Line::from(""));
                lines.push(Line::from(vec![
                    Span::styled(
                        " v ",
                        Style::default().bg(Color::Rgb(100, 180, 255)).fg(Color::Black).bold(),
                    ),
                    Span::raw(format!(
                        " View terminals ({})",
                        app.terminal_entries.len()
                    )),
                ]));
            }
...
fn vt100_screen_to_lines(screen: &vt100::Screen) -> Vec<Line<'static>> {
...
pub fn render_terminal_screen(f: &mut Frame, app: &mut App, area: Rect) {
    match app.terminal_mode {
        TerminalMode::Single => render_terminal_single(f, app, area),
        TerminalMode::Multi => render_terminal_multi(f, app, area),
    }
}

fn render_terminal_single(f: &mut Frame, app: &mut App, area: Rect) {
    let pty_running = {
        let running = app
            .pty_running
            .lock()
    ...
    let terminal_content = vt100_screen_to_lines(&screen);
    let terminal = Paragraph::new(terminal_content);
    f.render_widget(terminal, inner_area);
}

fn render_terminal_multi(f: &mut Frame, app: &mut App, area: Rect) {
    let len = app.terminal_entries.len();
    // Clamp selection to valid range; clear selection if list is empty
    if len == 0 {
        app.terminal_list_state.select(None);
    } else {
        app.selected_terminal_index = app.selected_terminal_index.min(len.saturating_sub(1));
        app.terminal_list_state
            .select(Some(app.selected_terminal_index));
    }

    // Left: list of terminals | Right: terminal view
    let chunks = Layout::default()
        .direction(Direction::Horizontal)
        .constraints([
    ...
    let pending = app.terminal_queue_state.pending_count();
    let brand_green = Color::Rgb(214, 255, 98);

    // One row per terminal entry (no fixed min/max)
    let rows: Vec<Row> = app
        ...
        .map(|(idx, entry)| {
            let task_name = app
                ...
                .map(|t| truncate(&t.node_id, 14))
                .unwrap_or_else(|| "?".to_string());
            let status = match &entry.slot {
                Some(s) if s.is_running() => "Running",
                Some(s) => match s.exit_code() {
                    Some(0) => "Done",
                    Some(_) => "Failed",
                    None => "Done",
                },
            ...
            let is_selected = app.selected_terminal_index == idx;
            let row_style = if is_selected {
                Style::default().fg(brand_green)
            } else {
                Style::default()
            ...
            Row::new(vec![
                ...
                Cell::from(task_name).style(row_style),
    ...
    let header_cells = ["", "#", "Task", "Status"]
        .iter()
        .map(|h| Cell::from(format!(" {} ", h)).style(Style::default().fg(brand_green).bold()));
    let header_row = Row::new(header_cells)
        .height(1)
        .bottom_margin(1)
        .style(Style::default().add_modifier(Modifier::BOLD));
    ...
    f.render_stateful_widget(table, list_area, &mut app.terminal_list_state);

    let slot_rows = terminal_area.height.saturating_sub(2);
    let slot_cols = terminal_area.width.saturating_sub(2);
    app.last_slot_size = (slot_rows, slot_cols);
    resize_slots(app, slot_rows, slot_cols);
    ...
    if let Some(slot) = entry {
        ...
        let parser = slot.parser.read().unwrap_or_else(|p| p.into_inner());
        let screen = parser.screen();
        let content = vt100_screen_to_lines(&screen);
        ...
        let block = Block::default()
            ...
            .border_type(BorderType::Rounded)
            .title(Span::styled(
                format!(" {} — {} ", app.selected_terminal_index + 1, task_name),
                Style::default().bold(),
            ))
            .border_style(Style::default().fg(Color::DarkGray));
    ...
    let hints = match app.screen {
        ...
        Screen::Tasks => {
            if app.terminal_entries.is_empty() {
                ...
                " ▲/▼ Navigate • Enter Select • a Trigger All • v Terminals • Esc Back • r Refresh • ? Help • q Quit "
            }
        }
        Screen::Actions => {
            " ▲/▼ Scroll • t Trigger • a Trigger All • Esc Back • r Refresh • ? Help • q Quit "
        }
        Screen::Terminal => {
            if app.insert_mode {
                " Type to input • Enter Submit • Ctrl+C Interrupt • Esc Exit Insert Mode "
            } else if app.terminal_mode == TerminalMode::Multi {
                " ▲/▼ Select terminal • 1-4 Jump • i Insert • Esc Back • q Quit "
            } else {
    ...
    match &app.popup {
        ...
        Popup::Help => {
            ...
            let text = vec![
                ...
                Line::from("  t                Trigger current task (opens terminal list)"),
                Line::from("  a                Trigger all awaiting (opens terminal list)"),
                Line::from("  v                View terminals (Tasks screen, when terminals running)"),
