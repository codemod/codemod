...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct AuthoringWarning {
...
pub(crate) fn parser_backed_authoring_warnings(
    package_path: &Path,
    manifest: &CodemodManifest,
) -> Result<Vec<AuthoringWarning>> {
    ...
    for resolved in expected_workflow_paths(package_path, manifest)? {
        ...
        let workflow = parse_workflow_file(&resolved.path).map_err(|error| {
            anyhow!(
                "Failed to parse workflow file {}: {error}",
                resolved.path.display()
            )
        })?;
        collect_parser_backed_authoring_warnings(
            package_path,
            &resolved.path,
            &workflow,
            &mut warnings,
...
fn collect_parser_backed_authoring_warnings(
    package_path: &Path,
    workflow_path: &Path,
    workflow: &Workflow,
    warnings: &mut Vec<AuthoringWarning>,
) {
    let template_by_id = workflow
        .templates
        .iter()
    ...
    for node in &workflow.nodes {
        for step in &node.steps {
            collect_parser_backed_authoring_warnings_from_action(
                package_path,
                workflow_path,
                &step.action,
                &template_by_id,
...
fn collect_parser_backed_authoring_warnings_from_action(
    package_path: &Path,
    workflow_path: &Path,
    action: &StepAction,
    ...
    match action {
        StepAction::JSAstGrep(js_ast_grep) => {
            let Some(language) = js_ast_grep.language.as_deref() else {
                return;
            };
            if !is_parser_backed_language(language) {
                return;
            }
            if let Some(script_path) =
                resolve_js_ast_grep_script_path(package_path, workflow_path, &js_ast_grep.js_file)
            {
                let Ok(content) = fs::read_to_string(&script_path) else {
                    return;
                };
                if script_has_full_source_rewrite_pattern(&content) {
                    warnings.push(AuthoringWarning {
                        workflow_path: workflow_path.to_path_buf(),
                        message: format!(
                            "`{}` targets parser-backed language `{}` but appears to use file-level string rewriting. Prefer AST-selected edits with `node.replace(...)` and `root.root().commitEdits(...)`; reserve raw text rewrites for unsupported plain-text formats.",
                            script_path.display(),
                            language
                        ),
                    });
...
fn resolve_js_ast_grep_script_path(
    package_path: &Path,
    workflow_path: &Path,
    js_file: &str,
) -> Option<PathBuf> {
    let js_file_path = Path::new(js_file);
    if js_file_path.is_absolute() && js_file_path.is_file() {
        return Some(js_file_path.to_path_buf());
    }

    let workflow_relative = workflow_path.parent()?.join(js_file_path);
    if workflow_relative.is_file() {
        return Some(workflow_relative);
    }

    let package_relative = package_path.join(js_file_path);
    package_relative.is_file().then_some(package_relative)
}

fn is_parser_backed_language(language: &str) -> bool {
    matches!(
...
fn script_has_full_source_rewrite_pattern(content: &str) -> bool {
    const PATTERNS: &[&str] = &[
        "readFileSync(",
        "writeFileSync(",
        "fs/promises",
...
"#,
    ...
    #[test]
    fn parser_backed_authoring_warning_flags_full_source_rewrite() {
        let temp_dir = tempdir().unwrap();
        let manifest = manifest_with("@codemod/raw-source");
...
"#,
        ...
        fs::write(
            temp_dir.path().join("scripts/codemod.ts"),
            r#"export default async (root) => root.root().text().replace(/npm/g, "pnpm");"#,
        )
        .unwrap();

        let warnings = parser_backed_authoring_warnings(temp_dir.path(), &manifest).unwrap();
        assert_eq!(warnings.len(), 1);
        assert!(warnings[0]
            .message
            .contains("parser-backed language `toml`"));
    }

    #[test]
    fn parser_backed_authoring_warning_allows_ast_node_replacements() {
        let temp_dir = tempdir().unwrap();
        let manifest = manifest_with("@codemod/ast-edit");
        fs::create_dir_all(temp_dir.path().join("scripts")).unwrap();
        write_workflow(
            temp_dir.path(),
            r#"
...
"#,
        ...
        fs::write(
            temp_dir.path().join("scripts/codemod.ts"),
            r#"export default async (root) => root.root().commitEdits([node.replace("package-manager = \"pnpm@9.0.0\"")]);"#,
        )
        .unwrap();
