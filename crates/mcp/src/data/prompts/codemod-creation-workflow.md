...
- If a codemod touches imports, read `jssg-utils-instructions` before implementing import logic. Explicitly check whether `getImport`, `getAllImports`, `addImport`, `removeImport`, or `stringToExactRegexString` cover the task, and use them by default when they do.
