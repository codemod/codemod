...
- Use multi-step workflows when the migration spans multiple safe sequential or independent transformation surfaces, for example JS/TS source plus JSON manifests plus Gradle or Podfile edits.
  - If the files are related, keep the hop inside JSSG with `jssgTransform`. For example a codemod that changes React components along with imported CSS modules should use `jssgTransform(...)` to modify the adjacent CSS module rather than introducing a second shell step that needs to know the CSS-module path.
- If a migration spans multiple related files but remains AST-safe, keep the work in JSSG and prefer `jssgTransform` or related JSSG APIs over a custom shell command.
