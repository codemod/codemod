...
5. Before deep implementation, define and create fixtures for risky paths. Do not start broad transform logic until these fixtures exist. At minimum, cover:
   - positive rewrites
   - preserve/no-op cases
   - precedence-sensitive expressions
   - comment-sensitive statements
   - flag/parameter-driven branches
   - truthy/falsy value lattices for migrated props/options
   - import/runtime binding edge cases when relevant
...
- If a codemod touches imports or needs symbol-origin/runtime-binding checks, read `jssg-utils-instructions` before implementing that logic. Explicitly check whether `getImport`, `getAllImports`, `addImport`, `removeImport`, `stringToExactRegexString`, `getTopLevelImportBinding`, `getAllTopLevelImportBindings`, `isTypeOnlyImportBinding`, `findShadowingBinding`, `isNodeBoundToIdentifier`, or `isRuntimeImportBinding` cover the task, and use them by default when they do.
- Prefer the documented import-helper patterns when they fit:
  ...
  - match existing ESM/CJS style when adding imports
- If a transform reconstructs JS/TS output with strings, do an explicit semantic-preservation pass before implementation:
  - operator precedence
  - associativity
  - evaluation order / side effects
  - comment preservation
  - JSX/spread ordering semantics
  - runtime-vs-type symbol origin
...
- Preserve repository-local dependency and metadata conventions when working in an existing monorepo:
  - prefer workspace `catalog:` dependency specifiers when the repo already pins them
  - align `codemod.yaml` shape with nearby packages (`workflow` vs `workflows`, language targets, capabilities)
  - align `workflow.yaml` with nearby conventions for `semantic_analysis`, `include`, and `exclude` globs
