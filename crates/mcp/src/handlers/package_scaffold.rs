...
impl PackageScaffoldHandler {
    ...
    #[tool(
        description = "Scaffold a codemod package by delegating to the real `codemod init` CLI. Use this immediately after registry search shows there is no exact existing package for the requested migration. Relative paths are resolved against the detected workspace root instead of the MCP process cwd."
    )]
    ...
    async fn scaffold_package(
        ...
        let resolved = resolve_requested_paths(&request.path)?;
...
fn resolve_requested_paths(path: &str) -> anyhow::Result<ResolvedScaffoldPaths> {
    let requested = PathBuf::from(path);
    if requested.is_absolute() {
        let workspace_root = requested
            .parent()
            .map(Path::to_path_buf)
            .unwrap_or_else(|| PathBuf::from("/"));
        return Ok(ResolvedScaffoldPaths {
            workspace_root,
    ...
    let package_root = if requested == Path::new(".") {
        workspace_root.clone()
    } else {
        workspace_root.join(requested)
    };

    Ok(ResolvedScaffoldPaths {
        workspace_root,
        package_root,
    })
}

fn detect_workspace_root() -> anyhow::Result<PathBuf> {
    for key in ["CODEMOD_WORKSPACE_ROOT", "INIT_CWD", "PWD"] {
    ...
    Err(anyhow!(
        "Relative scaffold paths could not be resolved because the MCP server cwd is `/` and no workspace-root environment variable was available. Retry with an absolute `path`."
    ))
}
...
mod tests {
    ...
    async fn scaffold_tool_invokes_real_cli_shape() {
        ...
        let generated_root = temp_dir.join("generated");
        assert!(args.contains(&generated_root.display().to_string()));
    ...
    fn resolve_requested_paths_uses_pwd_for_relative_inputs() {
        ...
        let resolved = resolve_requested_paths("codemod-testing/example").unwrap();
        assert_eq!(resolved.workspace_root, temp_dir);
        assert_eq!(
            resolved.package_root,
            temp_dir.join("codemod-testing/example")
        );
